...
export const TourHeaderCell: FC<TourHeaderCellProps> = props => {
    ...
    const handleExpandClick = () => {
        props.setIsExpanded((arr: boolean[]) =>
            arr.map((val: boolean, i: number) => (i === props.tourNumber - 1 ? !val : val))
        );
    };
...
const TourCell: FC<{ tourResults: number[]; isExpanded: boolean }> = props => {
    ...
    const renderExpandedResults = () => {
        return props.tourResults.map(result => {
            return (
                <TableCell
                    key={`${props.tourResults.toString()}_${result}`}
    ...
    return (
        <>
            <TableCell sx={styles} align="center" variant="body">
                {props.tourResults.reduce((a, b) => a + b)}
            </TableCell>
...
export const TeamTableRow: FC<TeamTableRowProps> = props => {
    ...
    const renderTourCells = () => {
        return props.toursWithResults.map((tour, i) => {
            return (
                <TourCell
                    tourResults={props.toursWithResults[i]}
    ...
    return (
        <TableRow>
            ...
            >
                {' '}
                {props.isAdmin ? (
                    <Link to={`/game-answers/${props.gameId}/teamId/${props.teamId}`} className={`${classes.menuLink}`}>
                        {' '}
                        {props.teamName}{' '}
                    </Link>
                ) : (
