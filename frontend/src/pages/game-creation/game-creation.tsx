...
const GameCreator: FC<GameCreatorProps> = props => {
    ...
    if (teamsFromDB && (props.mode != 'edit' || chosenTeams) && isPageLoading) {
        teamsFromDB.sort((a: Team, b: Team) =>
            (chosenTeams?.includes(a.name) &&
                chosenTeams?.includes(b.name) &&
    ...
    const renderMatrixQuestionInputs = () => {
        return Array.from(Array(tempMatrixRoundsCount).keys()).map(roundIndex => {
            return (
                <div
                    className={classes.tourQuestionInputsWrapper}
                    key={`matrix_tour_${roundIndex + 1}`}
                >
                    <p className={classes.tourName}>{`Тур ${roundIndex + 1} — ${tempMatrixRoundNames?.[
                        roundIndex
                    ]}`}</p>
    ...
    const handleSubmit = async (event: React.SyntheticEvent) => {
        ...
        if (props.mode === 'creation') {
            ...
            await ServerApi.createGame(gameName, teamIds ?? [], chgkSettings, matrixSettings, gameAccessLevel).then(
                res => {
                    if (res.status === 200) {
    ...
    const renderRoundNameInputs = () => {
        return Array.from(Array(tempMatrixRoundsCount || matrixSettings?.roundsCount || 0).keys()).map(index => {
            return (
                <div
                    className={classes.tourNameWrapper}
                    key={`matrixTourName_${index}`}
                >
            ...
            case 'main':
                return (
                    <div className={classes.pageWrapper}>
                        ...
                        <form
                            className={classes.gameCreationForm}
                            onSubmit={handleSubmit}
                        >
                            <div className={classes.contentWrapper}>
                                <div className={classes.gameParametersWrapper}>
                                    {(props.mode !== 'edit' || (props.mode === 'edit' && chosenTeams)) &&
                                    teamsFromDB ? (
                                        <>
                                            <Input
                                                type='text'
                    ...
                    <div className={classes.pageWrapper}>
                        ...
                        <div className={classes.gameParamsWrapper}>
                            <div className={classes.toursCountWrapper}>
                                <label
                                    htmlFor='toursCount'
                                    className={classes.toursCountLabel}
                                >
                                    Количество туров
                                </label>
                                <input
                            ...
                            <div className={classes.questionsCountWrapper}>
                                <label
                                    ...
                                    className={classes.questionsCountLabel}
                                >
                                    Вопросов в туре
                                </label>
                                <input
    ...
    return isCreatedSuccessfully ? (
        <Redirect
            to={{ pathname: props.isAdmin ? '/admin/start-screen' : '/start-screen', state: { page: 'games' } }}
        />
    ) : (
        <PageWrapper>
            <Header
