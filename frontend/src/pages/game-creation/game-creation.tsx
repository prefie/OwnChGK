...
const GameCreator: FC<GameCreatorProps> = props => {
    ...
    const [tempQuizRoundTypes, setTempQuizRoundTypes] = useState<RoundType[] | undefined>();
                    ...
                    {Array.from(Array(tempQuizQuestionsCount).keys()).map(questionIndex => (
                        <div className={classes.questionInputWrapper} key={`question_input_wrapper_${questionIndex + 1}`}>
                            ...
                            <TextareaAutosize
                                ...
                                minRows={1}
                                value={
                                    tempQuizQuestions?.[roundIndex + 1][questionIndex] ||
                                    quizSettings?.questions?.[roundIndex + 1]?.[questionIndex]
                                }
    ...
    const handleSubmit = async (event: React.SyntheticEvent) => {
        ...
        if (props.mode === 'creation') {
            ...
            await ServerApi.editGame(oldGameId, gameName, chgkSettings, matrixSettings, quizSettings, gameAccessLevel).then(
                res => {
                    if (res.status === 200) {
                        setIsCreatedSuccessfully(true);
                    } else if (res.status === 409) {
    ...
    const handleToursCountChange = (event: React.ChangeEvent<HTMLInputElement>, mode: 'chgk' | 'matrix' | 'quiz') => {
    ...
    const handleQuestionsCountChange = (event: React.ChangeEvent<HTMLInputElement>, mode: 'chgk' | 'matrix' | 'quiz') => {
        if (+event.target.value <= 30) {
            switch (mode) {
                case 'chgk': {
    ...
    const setTourMatrixName = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    const setTourQuizName = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
        setTempQuizRoundNames(
            prevValue =>
                prevValue?.map((tourName, i) => {
                    if (i === index) {
                        return event.target.value;
    ...
    const setTourQuizBlitz = (event: React.ChangeEvent<HTMLInputElement>, index: number) => {
        setTempQuizRoundTypes(
            prevValue =>
                prevValue?.map((tourName, i) => {
                    if (i === index) {
                        return event.target.checked ? RoundType.BLITZ : RoundType.NORMAL;
                    } else {
                        return tourName;
                    }
                })
    ...
    const renderRoundMatrixNameInputs = () => {
            ...
            return (
                <div className={classes.tourNameWrapper} key={`matrixTourName_${index}`}>
                    ...
                    <CustomInput
                        ...
                        onChange={event => setTourMatrixName(event, index)}
    ...
    const renderRoundQuizNameInputs = () => {
        return Array.from(Array(tempQuizRoundsCount || quizSettings?.roundsCount || 0).keys()).map(index => {
            return (
                <div className={classes.tourNameWrapper} key={`quizTourName_${index}`}>
                    <div className={classes.tourNumber}>{index + 1}</div>
                    <CustomInput
                        type="text"
                        id="tour-name"
                        ...
                        onChange={event => setTourQuizName(event, index)}
                                ...
                                <div className={classes.gameParametersWrapper}>
                                    {(props.mode !== 'edit' || (props.mode === 'edit' && chosenTeams)) && teamsFromDB ? (
                                        <>
                                            ...
                                            <div className={classes.chgkWrapper}>
                                                <div className={classes.modeName}>Квиз</div>
                                                {!quizSettings ? (
                                                    <div
                                                        className={classes.addModeButton}
                                                        onClick={() => {
                                                            setSubmitted(false);
                                                            setPage('quiz-settings');
                                                        }}
                                                    >
                                                        <AddRounded
                                                            sx={{
                                                    ...
                                                    <div className={classes.iconsWrapper}>
                                                        ...
                                                        <IconButton
                                                            onClick={() => setIsDeleteQuizModalVisible(true)}
                                                            edge="end"
                                                            sx={{
                                                                '& .MuiSvgIcon-root': {
                                                                    color: 'var(--color-control-error-enabled)',
                ...
                return (
                    <div className={classes.pageWrapper}>
                        ...
                        <div className={classes.tourNamesWrapper}>
                            <Scrollbar>{renderRoundMatrixNameInputs()}</Scrollbar>
                        ...
                        <div className={classes.gameParamsWrapper}>
                            <div className={classes.toursCountWrapper}>
                                ...
                                <input
                                    ...
                                    placeholder="30"
                                    required={true}
                                    onChange={event => handleToursCountChange(event, 'quiz')}
                                />
                            </div>
                            ...
                            <div className={classes.questionsCountWrapper}>
                                ...
                                <input
                                    ...
                                    placeholder="30"
                                    required={true}
                                    onChange={event => handleQuestionsCountChange(event, 'quiz')}
                                />
                            </div>
                        </div>
                        ...
                        <div className={classes.gameParamsButtonsWrapper}>
                            <button
                                ...
                                onClick={() => {
                                    setTempQuizRoundNames(prevValue => {
                                        return Array.from(Array(tempQuizRoundsCount).keys()).map(
                                            i => prevValue?.[i] || quizSettings?.roundNames?.[i] || ''
                                        );
                                    });
                                    setTempQuizRoundTypes(prevValue => {
                                        return Array.from(Array(tempQuizRoundsCount).keys()).map(
                                            i => prevValue?.[i] || quizSettings?.roundTypes?.[i] || RoundType.NORMAL
                                        );
                                    });
                                    setPage('quiz-tours');
                                }}
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    setTempQuizRoundTypes(undefined);
            ...
            case 'quiz-tours':
                return (
                    <div className={classes.pageWrapper}>
                        ...
                        <div className={classes.tourNamesWrapper}>
                            <Scrollbar>{renderRoundQuizNameInputs()}</Scrollbar>

                            {submitted && tempQuizRoundNames?.filter(n => n === '').length ? (
                                <small
                                    style={{
                                        position: 'absolute',
                                        color: '#FF0000',
                                    ...
                                    if (!tempQuizRoundNames?.filter(n => n === '').length) {
                                        setQuizSettings(prevValue => {
                                            return {
                                                ...
                                                questions: tempQuizQuestions || prevValue || []
                                            } as GamePartSettings;
                                        });
            ...
            case 'quiz-questions':
                return (
                    <div className={classes.questionsWrapper}>
                        ...
                        <div className={classes.questionInputsWrapper}>
                            <Scrollbar>{renderQuizQuestionInputs()}</Scrollbar>
                        </div>
