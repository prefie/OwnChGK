...
beforeEach(() => {
    game = new Game('1', 'newGame', GameTypeLogic.ChGK);
    team = new Team('cool', '1');
    game.addTeam(team);
    game.addRound(new Round('1', 1, 2, 50, GameTypeLogic.ChGK));
    round = game.rounds[0];
...
test('Should_get_total_score_when_exist_right_answers', () => {
    ...
    const round = new Round('1', 1, 2, 50, GameTypeLogic.ChGK);
...
test('Should_get_right_score_table_for_one_team_when_two_rounds', () => {
    ...
    const round2 = new Round('1', 2, 1, 50, GameTypeLogic.ChGK);
    ...
    expect(game.getScoreTableForTeam(1)[team.name]).toStrictEqual([
        [questionCost, 0],
        [questionCost, 0],
    ]);
});
...
test('Should_get_team_answer_when_it_exist', () => {
    const round1 = game.rounds[0];
    const round2 = new Round('1', 2, 5, 50, GameTypeLogic.ChGK);
...
test('Should_create_questions_as_in_setting', () => {
    const round = new Round('1', 1, 5, 50, GameTypeLogic.ChGK);
...
test('Should_accept_appeal_for_all_team', () => {
    ...
    if (teamAnswer1 !== undefined) {
        expect(teamAnswer1.status).toBe(AnswerStatus.RIGHT);
        expect(teamAnswer1.score).toBe(question.cost);
    }
    if (teamAnswer2 !== undefined) {
        expect(teamAnswer2.status).toBe(AnswerStatus.RIGHT);
